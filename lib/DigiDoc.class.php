...
class Parser_DigiDoc {
    ...
    function Parse($xml, $XMLPart = '') {
        ...
        if (isset($xml2['SOAP-ENV:Body'])) $body = $xml2['SOAP-ENV:Body'];
        else $body = $xml2;
    ...
    function getDataFileBlockHashCoded($xml) {
        ...
        if ($this->format == 'SK-XML' || ($this->format == 'DIGIDOC-XML' && ($this->version == '1.1' || $this->version == '1.2'))) return $xml;
        ...
        if ($xml_datafile_element_base64_canonized == "") return $xml;
        ...
        $xml_datafile_element_hashcode_canonized = $this->getXMLDataFileElementCanonized($xml_datafile_element_base64_canonized, 'filesha1', $hash);

        if ($xml_datafile_element_hashcode_canonized == "") return $xml;
    ...
    function file2hash($xml) {
        if (preg_match("'ContentType\=\"HASHCODE\"'s", $xml)) {
            ...
            //2010.04, ahto. digidoc 1.0, 1.1 ja 1.2 puhul ei arvuta räsi, hoiame täiskujul
            if ($this->format == 'SK-XML' || ($this->format == 'DIGIDOC-XML' && ($this->version == '1.1' || $this->version == '1.2'))) {
                return $xml;
            }
            else {
...
class File {
    ...
    function DirMake($strPath) {
        ...
        if (file_exists($strPath)) return is_dir($strPath) ? DIR_ERR_OK : DIR_ERR_NOTDIR;
        ...
        if ($nRet == DIR_ERR_OK) return mkdir($strPath) ? DIR_ERR_OK : DIR_ERR_EMKDIR_3;
        return $nRet;
    }
